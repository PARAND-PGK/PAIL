"""
هندلرهای مربوط به «ارسال‌شده‌ها»: نمایش لیست پیام‌های ارسالی، باز کردن، حذف و ستاره‌دار کردن.
"""
...
router = Router(name="sent")

BOX = "sent"


@router.message(F.text == "📤 ارسال‌شده‌ها", IsAuthenticated(), StateFilter(None))
async def show_sent(message: Message, current_user) -> None:
    addresses = await get_addresses_for_user(current_user["id"])
    address_list = [row["address"] for row in addresses]

    messages = await get_sent(address_list)

    if not messages:
        await message.answer("📭 هنوز هیچ پیامی ارسال نکرده‌اید.")
        return

    await message.answer(f"📤 <b>ارسال‌شده‌ها</b> ({len(messages)} پیام)")
...
async def delete_message_from_list(callback: CallbackQuery) -> None:
    ...
    await delete_message_for_sender(message_id)
...
async def back_from_detail(callback: CallbackQuery) -> None:
    await callback.message.edit_text("↩ برای دیدن لیست پیام‌ها دوباره از منو «📤 ارسال‌شده‌ها» را بزنید.")
